                ...
                box(title = "Configurare Analiză Socio-Demografică", status = "primary", solidHeader = TRUE, width = 12,
                    ...
                    column(4,
                           selectInput("socio_ref_country", "Compară cu:",
                                       choices = c("Media României" = "RO", "Media UE" = "EU",
                                                   "Germania" = "DE", "Franța" = "FR",
...
server <- function(input, output, session) {
  ...
  data_raw <- reactive({
    ...
    if (grepl("\\.xlsx?$", input$file$name, ignore.case = TRUE)) {
      if (requireNamespace("readxl", quietly = TRUE)) readxl::read_excel(fp)
      else { showNotification("Instalați pachetul readxl.", type = "warning"); read.csv(fp) }
    } else {
      read.csv(fp)
    }
  })
  ...
  output$tbl_col_types <- renderDT({
    req(data_info())
    info <- data_info()
    df_types <- data.frame(Coloana = names(info$types), Tip = unlist(info$types),
                           stringsAsFactors = FALSE)
    datatable(df_types, options = list(pageLength = 15, dom = "t"),
  ...
  output$ui_dist_alerts <- renderUI({
    ...
    for (item in dal$imbalance) {
      alerts <- tagList(alerts,
                        div(class = "alert-box alert-red", icon("exclamation-triangle"),
    ...
    if (!is.null(sk) && !is.null(sk$skewness)) {
      ...
      scl <- if (abs(sv) > 1) "alert-red" else if (abs(sv) > 0.5) "alert-orange" else "alert-green"
      si  <- if (abs(sv) > 1) "Asimetrie puternică – distribuție non-normală"
      else if (abs(sv) > 0.5) "Asimetrie moderată" else "Distribuție aproape simetrică"
      alerts <- tagList(alerts,
                        div(class = paste("alert-box", scl), icon("chart-area"),
                            strong(" Asimetrie (Skewness): "), paste0(sv, " – ", si)))
      op  <- as.numeric(sk$outliers_pct)
      ocl <- if (op > 10) "alert-red" else if (op > 5) "alert-orange" else "alert-green"
      alerts <- tagList(alerts,
                        div(class = paste("alert-box", ocl), icon("dot-circle"),
                            strong(" Valori atipice (Outlieri): "),
                            paste0(sk$outliers_count, " valori (", op, "%) în afara intervalului IQR")))
    }
    if (length(alerts) == 0)
      alerts <- div(class = "alert-box alert-green", icon("check-circle"), " Nicio alertă distribuțională detectată.")
    alerts
  })
        ...
        fluidRow(
          ...
          if (!is.null(mr$disparate_impact))
            infoBox("Disparate Impact", mr$disparate_impact, icon = icon("not-equal"),
                    color = if (mr$disparate_impact < 0.8 || mr$disparate_impact > 1.25) "orange" else "green", width = 3),
  ...
  output$tbl_group_summary <- renderDT({
    ...
    datatable(df_sum, rownames = FALSE, options = list(dom = "t"), class = "metric-table display")
  })
  ...
  socio_result <- eventReactive(input$run_socio, {
    ...
    if (type == "age") {
      ...
      if (length(edu_col) == 0) { showNotification("Nu s-a detectat o coloană de educație.", type = "warning"); return(NULL) }
      group_col <- edu_col[1]
  ...
  output$ui_socio_comparison <- renderUI({
    ...
    if (is.na(ref_val)) return(div(class = "alert-box alert-orange",
  ...
  output$tbl_socio_summary <- renderDT({
    ...
    datatable(socio_result(), rownames = FALSE,
              options = list(pageLength = 15, dom = "tip"), class = "metric-table display")
  })
  ...
  output$plot_barplot <- if (has_plotly) plotly::renderPlotly({
    ...
    g <- ggplot(df_bar, aes(x = reorder(Grup, Diferenta), y = Diferenta, fill = Directie)) +
      ...
      labs(x = input$sensitive,
           y = paste("Diferența față de media globală (", round(gm, 1), ")"),
  ...
  output$dl_boxplot <- downloadHandler(
    ...
    content  = function(file) ggplot2::ggsave(file, plot = make_boxplot_gg(), width = 10, height = 6, dpi = 150)
  )
  output$dl_density <- downloadHandler(
    filename = function() paste0("density_", Sys.Date(), ".png"),
    content  = function(file) ggplot2::ggsave(file, plot = make_density_gg(), width = 10, height = 6, dpi = 150)
  )
  output$dl_barplot <- downloadHandler(
    filename = function() paste0("barplot_", Sys.Date(), ".png"),
    content  = function(file) ggplot2::ggsave(file, plot = make_barplot_gg(), width = 10, height = 6, dpi = 150)
  )
  ...
  output$ui_export_preview <- renderUI({
    req(input$run)
    div(class = "alert-box alert-green", icon("info-circle"),
